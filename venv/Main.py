        ...
        if "player_ID" in session:
            ...
            try:
                if session["player_ID"] == game_players[game_num][0] or session["player_ID"] == game_players[game_num][1]:
                    # if the player_ID is equal to one of the two IDs in this game, let the player join the game
...
def get_score(game_num):
    ...
    if len(game_boards) >= 1:
        try:
            ...
            L = [score[1], game_timers[game_num], score[0], game_boards[game_num].map.getMap()]
            return json.dumps(L)
        except:
            pass
